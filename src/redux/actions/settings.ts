...
>('settings/fetchAndSetData', async (localData, thunkAPI) => {
  try {
    const data = Object.keys(localData).length === 0 ? await fetchData() : localData;
    ...
    data.projections[selectedProjection].data.forEach((element: any) => {
      Object.assign(element, data.protein_data[element.identifier]);
    });

    const items: Item[] = [];
    const colorParamList: string[] = data.visualization_state.colorParamList ?? [];
    const keys = Object.keys(data.projections[selectedProjection].data[0].features).filter(
      (item: any) => !Number(data.projections[selectedProjection].data[0].features[item])
    );
    const colorKey = keys[1];
    const newData = data.projections[selectedProjection].data.map((item: any) => {
      const newItem = {
        ...item
    ...
    thunkAPI.dispatch(
      setKeyList(Object.keys(data.projections[selectedProjection].data[0].features))
    );
    thunkAPI.dispatch(setThreeD(data.projections[selectedProjection].dimensions === 3));
    thunkAPI.dispatch(setColorKey(data.visualization_state.colorKey ?? 'major_group'));
    thunkAPI.dispatch(setColorParam(data.visualization_state.colorParam ?? ''));
    thunkAPI.dispatch(setColorParamList(colorParamList));
    thunkAPI.dispatch(setSearchItems(data.visualization_state.searchItems ?? []));
    ...
    thunkAPI.dispatch(setErrorMessage('The data could not be fetched. Please check the files.'));
  }
});

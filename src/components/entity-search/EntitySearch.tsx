...
function SearchItem({ item, clickHandler }: { item: Item; clickHandler: (item: Item) => void }) {
  return (
    <div
      role="button"
...
export default function EntitySearch() {
  ...
  const groupedResults =
    data && data.length > 0
      ? data.reduce(
          (acc, item) => {
            const category = item.id!.startsWith(query) ? 'ID Matches' : item.category;
    ...
    if (
      !searchItems.find(
        (o: { id: string; name: string; category: string }) =>
          o.id === item.id && o.name === item.name && o.category === item.category
      )
    ) {
    ...
    >
      ...
      >
        ...
        >
          ...
          {closed ? (
            <ArrowDownIcon className="w-4 mx-2" onClick={() => setClosed(!closed)} />
          ) : (
            <XMarkIcon
              ...
              <Tag
                ...
                onClick={() =>
                  dispatch(
                    setSearchItems(
                      searchItems.filter(
                        (clicked: { id: string; name: string; category: string }) =>
                          clicked.id !== item.id ||
                ...
                {Object.entries(groupedResults).map(([category, items], ix) => (
                  <div key={ix}>
                    ...
                    {items.map((item: Item, index: number) => (
                      <SearchItem key={index} item={item} clickHandler={handleItemClick} />
                    ))}
                  </div>
                ))}
              </div>

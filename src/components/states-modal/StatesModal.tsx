...
export function StatesModal({ showStatesModal, setShowStatesModal, cameraRef }: any) {
  const dispatch = useAppDispatch();

  const { states, customFeatures, technique, colorParam, colorParamList, dataItems } =
      ...
      <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50">
        <div className="modal-container bg-white w-11/12 md:max-w-md mx-auto rounded-lg shadow-lg z-50 overflow-y-auto max-h-[90%] p-6">
          <div className="flex justify-between items-center pb-4">
            <h2 className="text-2xl font-bold text-gray-800">States Settings</h2>
            <button
              aria-label="Close"
          ...
          <div className="mt-4">
            <label htmlFor="newStateName" className="block text-sm font-bold text-gray-700">
              Add New State
            </label>
          ...
          <label className="block text-sm font-bold text-gray-700 mt-4">Existing States</label>
          <div className="mt-2">
            {states.length === 0 ? (
              <div className="text-sm text-gray-700 py-2">No states available.</div>
            ) : (

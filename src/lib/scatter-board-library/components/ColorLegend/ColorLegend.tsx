...
const EditColorModal: React.FC<EditColorModalProps> = ({
      ...
      return {
        ...
        color:
          featureName === 'NaN' && !existingCustomization
            ? '#000000'
            : // @ts-expect-error ts-migrate(2531) FIXME: Object is possibly 'null'.
              existingCustomization?.color || colorList[index % colorList.length],
        category: category
      };
    });
  };
  ...
  const [editList, setEditList] = useState<ParamCustomization[]>(initializeEditList);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setInitialEditList(initialList);
    }
  ...
  return (
    <div
      ...
      <div className="flex items-center">
        ...
        <div className="cursor-pointer pl-2" onClick={() => setIsEditModalOpen(true)}>
          <svg
            ...
            return (
              <li key={value}>
                <ColorLegendItem
                  ...
                  color={value === 'NaN' && !customization ? '#000000' : displayColor}
                  text={displayName}

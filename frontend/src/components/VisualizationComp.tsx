...
const VisualizationComp = () => {
  ...
  //         new Vector3(
  //           -0.05792004425389764,
  //           0.0008789660095279305,
  //           0.000050966749574538594
  //         )
  //       )
  //     );
  //   } else {
  ...
  function importFileV2(parsedObject: any) {
    try {
      ...
      if (
        !parsedObject.projections ||
        !Array.isArray(parsedObject.projections)
      ) {
        dispatch(
          setErrorMessage(
            "Invalid JSON: 'projections' is missing or not an array."
          )
        );
        throw new Error(
          "Invalid JSON: 'projections' is missing or not an array."
      ...
      const selectedProjection =
        parsedObject.visualization_state?.technique ?? 0;

      if (!parsedObject.projections[selectedProjection]) {
        dispatch(
          setErrorMessage(
            `Invalid JSON: Projection at index ${selectedProjection} does not exist.`
          )
        );
        throw new Error(
          `Invalid JSON: Projection at index ${selectedProjection} does not exist.`
      ...
      if (!projectionData || !Array.isArray(projectionData)) {
        dispatch(
          setErrorMessage(
            "Invalid JSON: 'data' within projections is missing or not an array."
          )
        );
        throw new Error(
          "Invalid JSON: 'data' within projections is missing or not an array."
      ...
      if (parsedObject.protein_data) {
        projectionData.forEach((element: any, index: number) => {
          ...
          if (
            ...
            typeof parsedObject.protein_data[element.identifier].features !==
              "object"
          ) {
            dispatch(
              setErrorMessage(
      ...
      dispatch(
        setColorParam(parsedObject.visualization_state?.colorParam ?? "")
      );
      dispatch(
        setSearchItems(parsedObject.visualization_state?.searchItems ?? [])
      );
      ...
      // Optionally, you can dispatch an error action or show a notification to the user
                ...
                <ScatterBoard
                  ...
                  onVisualizeClicked={(value: string) => {
                    if (settings.selectedMols.length === 2) {
                      let selectedMolsCopy = [...settings.selectedMols];
                      selectedMolsCopy[0] = value;
                      dispatch(setSelectedMols(selectedMolsCopy));
                      setIsCompareMode(true);
                    }
                    if (
                      !(value in settings.selectedMols) &&
                      settings.selectedMols.length < 2
                    ) {
                  ...
                  onCompareClicked={(value: string) => {
                    if (settings.selectedMols.length === 0) {
                      setErrorMessage(
                        "Please select a molecule to compare with."
                      );
                      return;
                    }

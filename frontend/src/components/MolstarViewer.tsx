...
const MolstarViewer = ({ isCompareMode = false }) => {
  ...
  useEffect(() => {
    async function init() {
      if (isCompareMode && selectedMols.length >= 2) {
        window.molstar1 = await createPluginUI(
          viewer1Ref.current as HTMLDivElement,
          MySpec
        );
        window.molstar2 = await createPluginUI(
          viewer2Ref.current as HTMLDivElement,
          MySpec
        );
  ...
  async function loadMolecule(plugin: PluginUIContext, moleculeName: string) {
    ...
    plugin.builders.structure.plugin.config.set(
    ...
    const provider =
      plugin.builders.structure.representation.resolveProvider(preset);
